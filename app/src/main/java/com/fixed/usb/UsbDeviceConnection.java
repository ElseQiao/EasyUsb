...
/**
 * This class is used for sending and receiving data and control messages to a
 * USB device. Instances of this class are created by
...
public class UsbDeviceConnection {
     ...
     * @hide
     */
    public UsbDeviceConnection(UsbDevice device, int device_fd) {
        mDevice = device;
        native_open(device.getDeviceName(), device_fd);
    }
    ...
    /**
     * Returns the native file descriptor for the device, or -1 if the device is
     * not opened. This is intended for passing to native code to access the
     * device.
     *
     ...
     * @param buffer      buffer for data portion of transaction, or null if no data
     ...
     * {@link UsbEndpoint}s. When multiple endpoints are in
    ...
    private native boolean native_open(String deviceName, int device_fd);

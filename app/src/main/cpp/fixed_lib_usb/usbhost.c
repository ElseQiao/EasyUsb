...
#define MAX_USBFS_BUFFER_SIZE   16384*4
...
//                D("%s subdirectory %s: index: %d\n", (event->mask & IN_CREATE) ? "new" : "gone", path, i);
...
{
    //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 1");

    if (!device || NULL == device)
    {
        //LOGE("usb_device_reopen_writeable -- 1.2");
        return -1;
    }

    //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 1.3");

    int a = device->writeable;

    /*if(DEBUG)
    {
        //LOGE("usb_device_reopen_writeable -- 1.4.1");
        //LOGE("usb_device_reopen_writeable -- 1.4.2 == %d", a);
    }*/

    if (device->writeable)
    {
        //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 2");
        return 1;
    }

    //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 3");

    int fd = open(device->dev_name, O_RDWR);

    //if(DEBUG)  //LOGE("usb_device_reopen_writeable -- 4");

    if (fd >= 0)
    {
        //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 5");
    ...
    //if(DEBUG) //LOGE("usb_device_reopen_writeable -- 6");
    return 0;
}

int usb_device_get_fd(struct usb_device *device)
{
    //if(DEBUG) //LOGE("usb_device_get_fd -- 1");

    if (!usb_device_reopen_writeable(device))
    {
        //LOGE("usb_device_get_fd -- 2");
        return -1;
    }

    //if(DEBUG) //LOGE("usb_device_get_fd -- 3");
                                     ...
                                     unsigned int interface, int connect)
{
    ...
    ctl.data = NULL;
    return ioctl(device->fd, USBDEVFS_IOCTL, &ctl);//0213101688
}
...
{
    //if(DEBUG) //LOGE("native usb_request_queue 1");

    struct usbdevfs_urb *urb = (struct usbdevfs_urb*)req->private_data;
    int res;

    //if(DEBUG) //LOGE("native usb_request_queue 2");

    urb->status = -1;
    urb->buffer = req->buffer;

    //if(DEBUG) //LOGE("native usb_request_queue 3");
    ...
    //if(DEBUG) //LOGE("native usb_request_queue 4");
    ...
    //if(DEBUG) //LOGE("native usb_request_queue 5");
...
{
    //if(DEBUG) //LOGE("usb_request_wait 1");

    struct usbdevfs_urb *urb = NULL;
    struct usb_request *req = NULL;

    //if(DEBUG) //LOGE("usb_request_wait 2");

    while (1)
    {
        //if(DEBUG) //LOGE("usb_request_wait 3");

        int res = ioctl(dev->fd, USBDEVFS_REAPURB, &urb);

        //if(DEBUG) //LOGE("usb_request_wait 4 -- %d", res);

        if (res < 0)
        {
            //if(DEBUG) //LOGE("usb_request_wait 5");

            if(errno == EINTR)
            {
                //if(DEBUG) //LOGE("usb_request_wait 6 -- errno == EINTR");
                continue;
            }

            //if(DEBUG) //LOGE("usb_request_wait 7");
...
//            if(DEBUG)
            //LOGE("usb_request_wait mem : %p",req->client_data);
        }

        break;
    }

    //if(DEBUG) //LOGE("usb_request_wait 9");
